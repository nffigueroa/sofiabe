...
var clientMiddleware = {
    ...
    updateClient: async function updateClient(req, res, next) {
        var payLoad = {
        ...
        var client = Object.assign(payLoad, req.body);
        console.log(client);
        var idClient = req.params.idClient;
        var response = await (0, _network.callSPWithCallback)('Call CLI_eliminar_Cliente(?, ?, ? , ?, ? , ?, ?)', client).then(function (response) {
            if (response) {
                return (0, _response.ResponseBodyBuilder)(200, false, !!response);
            } else {
                return (0, _response.ResponseBodyBuilder)(404, true, 'Can not find id ' + idClient);
